...
public class ExamQuestion {
	...
	public ExamQuestion(String i, String t1, String t2, String c, String a) {
		this.qid = i;
		this.type1 = t1;
		this.type2 = t2;
		this.content = c;
	...
	public static boolean isExistQues(String dbName, String qid) {
		ArrayList<ExamQuestion> allQues = getAllQues(dbName);
		ExamQuestion ques = null;
		for (int i = 0; i < allQues.size(); i++) {
			ques = (ExamQuestion) allQues.get(i);
			if (ques.getQid() != null) {
	...
	public static ArrayList<?> getTypes(String dbName, String col) {
		System.out.println("getType and col="+col);
		String sql = "SELECT " + col + ", COUNT(*) AS NUM FROM ExamDB GROUP BY "+col+";";
	...
	public static ExamQuestion getQues(String dbName, String qId) {
		...
		if (result.size() > 0) {
			...
			ques = new ExamQuestion(data[0], data[1], data[2], data[3], data[4]);
		}
		return ques;
	}

	public static ArrayList<ExamQuestion> getAllQues(String dbName) {
		System.out.println("getAllQues");
		ArrayList<ExamQuestion> allQues = new ArrayList<ExamQuestion>();
		ExamQuestion ques = null;
		;
	...
	public static ArrayList<ExamQuestion> getSomeQues(String dbName, String col, String[] type) {
		System.out.println("getSomeQues");
		ArrayList<ExamQuestion> someQues = new ArrayList<ExamQuestion>();
		ExamQuestion ques = null;
		String[] data = null;
		...
		sql = "SELECT * FROM ExamDB WHERE "+condition+" order by QID;";
		String[] param = new String[1];
		param[0] = condition;
		//ArrayList<?> result = DbProxy.selectData(dbName, sql, param);
		ArrayList<?> result = DbProxy.selectData(dbName, sql);
