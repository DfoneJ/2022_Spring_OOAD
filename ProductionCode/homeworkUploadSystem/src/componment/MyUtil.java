...
public class MyUtil {
	...
	public static void printTeacherHead(PrintWriter pw) {
		...
		pw.println("function checkString(s1,s2) {  if ((s1==\"\")||(s2==\"\")) { alert(\"請填完整\");  return false;  } }  ");
		pw.println("function checkClearExam(s1,s2) {  if ((s2[0].checked)&&(s1.length==0)) { alert(\"請填完整\");  return false;  } ");
...
//		pw.println("function validateNumber(s1){");
//		pw.println("  if( !(/^\\d+$/.test(s1)) ){   ");
//		pw.println("    alert(\"某些欄位不允許輸入非數字字串\"); return false;  }}");
