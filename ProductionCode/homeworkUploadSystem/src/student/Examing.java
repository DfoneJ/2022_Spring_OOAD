...
public class Examing extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		userExam = (StudentExam) session.getAttribute("userExam");
		// --------- 檢查是否超過考試時間 -------------------------
		...
		//System.out.println("Examing dbname:"+dbName);
		...
		//System.out.println("thisQuesCount:"+thisQuesCount);
		//System.out.println("userCurrent:"+userCurrent);
		//System.out.println("newQuesSet:"+newQuesSet);
		//System.out.println("userExam.getQuesCount():"+userExam.getQuesCount());
		int questionNumber = userCurrent-4;
		for (int i = 1; i <= thisQuesCount; i++) {
			pw.println("   if (f.rdoQ" + i + "[4].checked) {       ");
			pw.println("         alert(\"第" + questionNumber + "題放棄\");       ");
		...
		pw.println(userExam.getUserCurrent() + " " + st.getName() + " 同學,答過答案: " + userExam.getUserAnswer() + ", 本次共 "
		...
		for (int i = 1; i <= newQuesSet; i++) {
			...
			for (j = 1; j <= opt.length; j++) {
				pw.println("<INPUT type=radio name=" + n + " value=" + opt[j - 1] + ">");
