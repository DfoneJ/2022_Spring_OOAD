...
public class TLogin extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		pw.println("課程選擇 : ");
		pw.println("<select name=rdoCourse>");
		for (int j = 1; j <= optName.length; j++) {
			pw.println("<option value="+ j +" >" + optName[j - 1] + "</option>");
		}
		pw.println("</select>");
		pw.println("<p><p>");
		...
		pw.println(
				"<input type = submit onClick=\"return checkString(FORM1.name.value, FORM1.passwd.value);\"  style=width:60 value =Ok>");
		pw.println("</form>");
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if (managerIp.contains(req.getRemoteAddr().toString())) {
			if ((!realName.equals("密碼錯誤")) && (!realName.equals("查無此人"))) {
				...
				Course session_course = new Course(course, course, userName, userName); // OOAD FINAL
				session.setAttribute("Teacher", teacher);
				session.setAttribute("Course", session_course);
