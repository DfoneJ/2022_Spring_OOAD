...
public class ChooseQuesType extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		ArrayList<?> choose = ExamQuestion.getTypes(tr.getDbName(), "Type1");
		String[] data = null;
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		//System.out.println("chooseNo="+chooseNo);
		type = new String[No];
		for (int i = 0, k = 0; i < chooseNo; i++) {
			if (s[i] != null) {
				type[k] = s[i];
				// pw.println(type[k]+"<br>");
				//System.out.println("type[k]="+type[k]);
				k++;
			}
		}
		ArrayList<?> allQues = ExamQuestion.getSomeQues(tr.getDbName(), "Type1", type);
		//System.out.println("hereF size="+allQues.size());
