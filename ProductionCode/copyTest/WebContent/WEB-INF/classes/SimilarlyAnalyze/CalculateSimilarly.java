...
public class CalculateSimilarly {
	...
	private ArrayList<ArrayList<String>> textSimilarityTable = new ArrayList<ArrayList<String>>();// 文字相似度
	private ArrayList<ArrayList<String>> structureSimilarityTable = new ArrayList<ArrayList<String>>();// 僅有DCS
																										// Tree的結構相似度
	private ArrayList<ArrayList<String>> variableSimilarityTable = new ArrayList<ArrayList<String>>();// 變數相似度
	private ArrayList<ArrayList<String>> ooSimilarityTable = new ArrayList<ArrayList<String>>();// 物件導向特性相似度
	private ArrayList<ArrayList<String>> totalSimilarityTable = new ArrayList<ArrayList<String>>();// 總合相似度(無扣除不符Threshold，亦無排序)
	private ArrayList<ArrayList<String>> highSimilarityList = new ArrayList<ArrayList<String>>();// 排序過後的textSimilarityTable
	private ArrayList<ArrayList<String>> DCSandOOSimilarityTable = new ArrayList<ArrayList<String>>();// 整合DCS
																										// Tree與物件導向特性分析的結構相似度
																										// by
																										// hongzhan
	private ArrayList<ArrayList<String>> finalSimilarityTable = new ArrayList<ArrayList<String>>();// 扣除不符合Threshold後的總合相似度
																									// by
																									// hongzhan
	...
	public void degreeOfPlagiarismUsingTextAnalyze(int topNum,
		...
		for (int i = 0; i < orderList.size() - 1; i++) // 結果排序
		{
			for (int j = i + 1; j < orderList.size(); j++) {
				if (Double.parseDouble(orderList.get(i).get(2)) < Double
					...
					orderList.remove(i);
					orderList.add(i, orderList.get(j - 1));
					orderList.remove(j);
					orderList.add(j, tmp);
				}
			}
		...
		// ricehuang 2011-04-08
		{
			// System.out.print(fileList.get(Integer.parseInt(textSimilarityTable.get(i).get(0))).get(0)
			// + "      ");
			// System.out.print(fileList.get(Integer.parseInt(textSimilarityTable.get(i).get(1))).get(0)
			// + "      ");
			// System.out.println("相似度：" +
			// nf.format(Double.parseDouble(textSimilarityTable.get(i).get(2))));
	...
	public void degreeOfPlagiarismUsingStructureAnalyze(int topNum,
		...
		nf.setMaximumFractionDigits(3);// 若小數點超過三位，則第四位~四捨五入 by hongzhan
		...
		for (int i = 0; i < expressionArrayList.size() - 1; i++) // 程式比對(使用structure
																	// analyze)
		{
		 ...
		 * //結果排序 { for(int j = i+1; j < this.structureSimilarityTable.size();
		 * j++) {
		 ...
		 * .get(i).get(1))).get(0) + "      "); System.out.println("相似度：" +
		 * nf.format
	...
	public void degreeOfPlagiarismUsingVariableAnalyze(int topNum,
		...
		/*
		 * for(int i = 0; i < this.variableSimilarityTable.size()-1; i++) //結果排序
	...
	public void degreeOfTotalAnalyzePlagiarism(int topNum,
		...
		for (int i = 0; i < this.totalSimilarityTable.size() - 1; i++) // 結果排序
		{
		 ...
		 * //印出相似度高的file {
		 * System.out.print(fileList.get(Integer.parseInt(totalSimilarityTable
		 * .get(i).get(0))).get(0) + "      ");
		 * System.out.print(fileList.get(Integer
		 * .parseInt(totalSimilarityTable.get(i).get(1))).get(0) + "      ");
		 * System.out.println("相似度：" +
		 * nf.format(Double.parseDouble(totalSimilarityTable.get(i).get(2)))); }
		 */
	...
	/* 扣除相似度低於Thresholds的Pair並且計算平均 by hongzhan */
	...
	/* 排序Total Similarly by hongzhan */
	public ArrayList<ArrayList<String>> sortTotalSimilarly(
			SettingInformation settingInformation) {
		for (int i = 0; i < this.finalSimilarityTable.size() - 1; i++) // 結果排序
		{
