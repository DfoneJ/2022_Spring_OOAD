...
public class Parser {
	    ...
	    for( j = 0 ; j < i ; j++)
	    {
	    	...
	    	else if(tokenArrayList.get(j).getName().equals("#"))	//將include合成一個token
	        {
	    		...
	    		{ 
	        		...
	        		i = i - 3;
	        	//	j = j + 3;//? by hongzhan
	    		}//僅限#define PI 3.14159此種形式，#difine max(x,y) ((x)>(y)?(x):(y))及#define _UNICODE 會亂 by hongzhan
	        }
	...
	public void setLine(ArrayList<Token> tokenArrayList)
	{
		...
		while(index < tokenArrayList.size())
	    {
			 ...
			 */
			/*
			 * 遇到}行數也加一，但像{{1,2},{2,3}} {}; do{}while();這三種情況則不換行
			 * do{}while();標準格式為
			 * do
			 * {
			 * }while();
			 ...
			 * include和define
			 */
			//此為getType，所以其token已在setReservedWordType method中整合為像#include<stdio>的樣式 by hongzhan
	...
	{	
		for( int index = 0; index < tokenArrayList.size(); index++)
		{
			...
			if(isDataType(tokenArrayList.get(index).getName()) /*&& tokenArrayList.get(index).getLevel() == 0*/)
			{
				...
				if(tokenArrayList.get(++index).getType().equals("Letter") && tokenArrayList.get(index+1).getName().equals("("))	//確定是function
				{
	...
	public void addVariableInfunction(ArrayList<Token> tokenArrayList)
	{
		for(int index = 0 ; index < tokenArrayList.size() ; index++)
		{
			if(tokenArrayList.get(index).getType().equals("function"))	//紀錄function內參數
			{
			...
			else if(tokenArrayList.get(index).getType().equals("class"))	//紀錄class內資訊
			{
